...
(function(){
  ...
  async function syncSmart(){
    // Ensure server folder exists for month+category, then refresh (even if ensure fails).
    const month = selectedMonth || curMonth();
    const cat = selectedCat || 'incoming';
    let fid = '';
    try{
      fid = await ensureSmartFolder(month, cat);
    }catch(err){
      console.warn('[vault] ensureSmartFolder failed', err);
      // Don't block the user from seeing existing folders/files.
    }
    await refresh(fid || '');
  }
  ...
  async function refresh(selectFolderId){
    ...
    if (sel){
      const month = selectedMonth || curMonth();
      const cat = selectedCat || 'incoming';
      const catDef = (VAULT_CATS||[]).find(x=>String(x.id||'')===String(cat||''));
      const catName = catDef ? TT(catDef.labelKey || ('vault.tabs.'+catDef.id), null, catDef.fallback || catDef.label || catDef.id) : String(cat||'');
      ...
      const softMatch = (f)=>{
        ...
        if (mstr && nm.indexOf(mstr) === -1) return false;
        const kw = {
          incoming: ['вход', 'zakup', 'zakupy', 'incoming'],
          outgoing: ['выстав', 'doch', 'dochod', 'outgoing', 'issued'],
          tax: ['zus', 'pit', 'podat', 'tax'],
          proof: ['подтверж', 'potwier', 'proof', 'payment'],
          other: ['другое', 'inne', 'other']
        };
        const arr = kw[String(cat||'')] || [];
        if (!arr.length) return false;
        return arr.some(k=>nm.indexOf(String(k)) !== -1);
      };
      const inMonthCat = (f)=>{
        const m = f && f.meta ? String(f.meta.month||'') : '';
        const c = f && f.meta ? String(f.meta.category||'') : '';
        if (m === month && c === cat) return true;
        // Fallback for old folders created without meta
        if ((!m && !c) || (m === month && !c) || (!m && c === cat)) return softMatch(f);
        return false;
      };
      const label = (f)=>{
        if (!f) return '';
