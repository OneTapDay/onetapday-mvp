...
(function(){
  ...
  async function syncSmart(){
    // Always load current state first so folders/files are visible even if smart-folder auto-create fails.
    const month = selectedMonth || curMonth();
    const cat = selectedCat || 'incoming';

    // 1) Load existing folders/files (never blocked by ensureSmartFolder)
    try{
      await refresh('');
    }catch(err){
    ...
    // 2) Try to ensure/create the smart folder. If it fails, we fall back to an existing folder by meta.
    let fid = '';
    let ensured = false;
    try{
    ...
    if (!fid){
      fid = folderByMeta(month, cat);
      if (!fid){
        // nothing to auto-select, keep whatever is selected
        setStatus(TT('vault.status.select_folder', null, 'Select a folder or create a new one.'));
        return;
      }
    }

    // 3) If ensure created a new folder, we need to reload state so it appears in the select.
    const exists = (vaultState && Array.isArray(vaultState.folders)) ? vaultState.folders.some(f=>f && f.id===fid) : false;
    if (ensured && fid && !exists){
      try{ await refresh(fid); }catch(_){ /* ignore */ }
      return;
    }

    // 4) Select in UI without throwing scary errors
    try{
